    ...
    <div w-class="item" on-tap="itemClick(e,{{i}})">
        ...
        <div w-class="right-container">
            <div w-class="top-container">
                <div w-class="currency-name">{{v.currencyName === 'KT' ? it.ktShow : (v.currencyName === 'ST' ? it.stShow : v.currencyName)}}</div>
                <div w-class="balance">{{v.balance%1===0?v.balance.toFixed(2):v.balance}}</div>
            </div>
            <div w-class="bottom-container">
                <div w-class="description">&nbsp;≈{{it.currencyUnitSymbol}}{{v.rate}}/{{v.currencyName === 'KT' ? it.ktShow : (v.currencyName === 'ST' ? it.stShow : v.currencyName)}}</div>
