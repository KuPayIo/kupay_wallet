...
import { callFetchCloudWalletAssetList, getStoreData } from '../../../middleLayer/wrap';
...
export class MyHome extends Widget {
    ...
    public updateLocalWalletAssetList() {
        callFetchCloudWalletAssetList().then(assetList => {
            this.props.wallet = [
            { num:assetList[1].balanceValue,name:`${WALLET.SC_SHOW}余额` },
            { num:assetList[0].balance,name:`我的${WALLET.KT_SHOW}` }];
            this.paint();
        });
    }
...
// 云端余额变化
registerStoreData('cloud/cloudWallets',(r) => {
