    ...
    <div w-class="content" on-scroll="pageScroll" id="redEnvDetail">
        ...
        <div w-class="topBack">
            ...
            <div style="margin: 30px 0 10px;font-size: 30px;color: #222222;">
                {{_cfg.defaultUserName}}
                {{if it1.showPin}}<span w-class="other">{{_cfg.pin}}</span>{{end}}
            </div>
            <div>{{it1.message}}</div>
        </div>
        <div w-class="bottom">
            <div w-class="tips">{{_cfg.tips[0] + it.curNum + "/" + it.totalNum}}，{{_cfg.tips[1] + it.amount + it.ctypeShow}}</div>
            ...
            <div w-class="endMess">
                <div w-class="againSend" on-tap="againSend">{{_cfg.tips[2]}}</div>
                <div>{{_cfg.tips[3]}}</div>
            </div>
        </div>
