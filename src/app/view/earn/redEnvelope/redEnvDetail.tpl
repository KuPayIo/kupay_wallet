<div class="new-page" ev-back-click="backPrePage" w-class="new-page">
    ...
    <div w-class="content" on-scroll="pageScroll" id="redEnvDetail">
        ...
        <div w-class="bottom">
            <div w-class="tips">{{it1.cfgData.tips[0] + it.curNum + "/" + it.totalNum}}，{{it1.cfgData.tips[1] +
                it.amount + it.ctypeShow}}</div>
            {{for ind,val of it1.redBagList}}
            <app-components-fourParaImgItem-fourParaImgItem>{name:{{val.userName}},data:{{val.amount+"
                "+it.ctypeShow}},time:{{val.timeShow}},img:{{val.avatar}},describe:{{it1.greatUser==ind?it1.cfgData.greatUser:""}}
                }</app-components-fourParaImgItem-fourParaImgItem>
            {{end}}

            {{if it.curNum < it.totalNum && !it.outDate}} <div w-class="endMess">
                <div w-class="againSend" on-tap="againSend">
                    <pi-ui-lang>{"zh_Hans":"继续发送","zh_Hant":"繼續發送","en":""}</pi-ui-lang>
                </div>
                <div>
                    <pi-ui-lang>{"zh_Hans":"24小时未领取的红包将退回云账户","zh_Hant":"24小時未領取的紅包將退回雲賬戶","en":""}</pi-ui-lang>
                </div>
        </div>
        {{end}}
    </div>
