<div class="new-page" ev-back-click="backPrePage" w-class="new-page">
    {{if !it1.isScroll}}
    <app-components1-topBar-topBar>{"title":{{_cfg.topBarTitle}},background:"#3EB4F1" }</app-components1-topBar-topBar>
    {{else}}
    ...
    <div w-class="content" on-scroll="getMoreList" id="exchangeHistoryContent">
        <div id="exchangeHistoryRecords" w-class="records">
            ...
            <div w-class="topBack">
                ...
                <div style="margin-top: 20px;">{{_cfg.tips[0]}}</div>
                <div style="margin-bottom: 90px;"><span style="font-size: 64px;">{{it1.convertNumberShow}}</span>{{_cfg.tips[1]}}</div>
            </div>
            <div w-class="bottom">
                {{if it1.recordListShow.length==0}}
                    <div style="text-align: center;">
                        <img src="../../../res/image/exchangeEmpty.png" style="width: 200px;height: 200px;margin-top: 210px;"/>
                        <div style="font-size: 32px;color: #888888;margin-top: 20px;">{{_cfg.tip[2]}}</div>
                    </div>
                {{else}}
                    <div w-class="tips">{{_cfg.tips[3]}}</div>
                    {{for ind,val of it1.recordListShow}}
                    <div on-tap="goDetail({{ind}})">
                        {{let desc=val.totalNum?val.curNum + "/" + val.totalNum + _cfg.tips[4]:""}}
                ...
                {{if it1.showMoreTips}}
                <div w-class="endMess" id="more">{{_cfg.tips[5]}}^_^</div>
                {{end}}
