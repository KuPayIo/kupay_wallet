...
{{else}}
    {{for i, v of it1.tabBarList}}
        <widget w-tag={{v.components}} style="visibility: {{v.modulName == it1.isActive ? 'visible' : 'hidden'}}; z-index:{{v.modulName == it1.isActive ? 0 : -1}}; position:absolute; width:100%;height:100%;">{isActive:{{v.modulName == it1.isActive}}}</widget>
    {{end}}
{{end}}
{{if it1.loading}}
...
<div w-class="ga-bottom-tab-bar-container">
    ...
    <div w-class="ga-tab-bar-item {{it1.isActive == item.modulName ? 'ga-tab-bar-item-active' : ''}}" on-down="tabBarChangeListener(e,{{index}})">
        <img src="../../res/image1/{{it1.isActive == item.modulName ? item.iconActive : item.icon}}" w-class="ga-tab-bar-icon" />
