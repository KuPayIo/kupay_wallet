 ...
 * 管理端登录
 */
export const admin_login = (username: string, password: string, cb: Function, timeout?: number) => {
	if (con === null) {
		if (conState !== ConState.init)
			throw new Error("login, invalid state: " + conState);
		return open(() => {
			admin_login(username, password, cb, timeout);
		}, (result) => {
			// callTime(cb, [result], "login");
			cb([result])
		}, timeout);
 ...
 * 代理商登录
 */
export const agent_login = (agent_id:number, cb: Function, timeout?: number) => {
	if (con === null) {
		if (conState !== ConState.init)
			throw new Error("login, invalid state: " + conState);
		return open(() => {
			agent_login(agent_id, cb, timeout);
		}, (result) => {
			// callTime(cb, [result], "login");
			cb([result])
		}, timeout);
	}
	con.request({ type: "agent_login", param: { "agent_id":agent_id} }, (result) => {
		if (result.error) {
			setLoginState(LoginState.logerror);
			result.result = result.error;
			// callTime(cb, [result], "logerror");
